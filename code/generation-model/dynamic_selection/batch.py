...
class Batch:
    ...
    def __init__(self, torch_batch, pad_index, model):
        ...
        :param use_cuda:
        """
        self.src1, self.src1_lengths = torch_batch.src1
        self.src1_mask = (self.src1 != pad_index).unsqueeze(1)
        self.src2, self.src2_lengths = torch_batch.src2
        self.src2_mask = (self.src2 != pad_index).unsqueeze(1)

        self.nseqs = self.src1.size(0)
        self.trg_input = None
        self.trg = None
        self.trg_mask = None
        ...
        if hasattr(torch_batch, "trg"):
            ...
            if self.future_prediction != 0:
                ...
                for i in range(0, self.future_prediction):
                    future_trg = torch.cat(
                        (future_trg, self.trg[:, i:-(self.future_prediction - i), :-1].clone()), dim=2)
            ...
            self.trg_mask = (F.pad(input=trg_mask.double(), pad=(
                pad_amount, 0, 0, 0), mode='replicate') == 1.0)
    ...
    def _make_cuda(self):
        ...
        :return:
        """
        self.src1 = self.src1.cuda()
        self.src1_mask = self.src1_mask.cuda()
        self.src2 = self.src2.cuda()
        self.src2_mask = self.src2_mask.cuda()
