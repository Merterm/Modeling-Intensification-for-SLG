...
from helpers import load_config, log_cfg, load_checkpoint, make_model_dir, \
    make_logger, set_seed, symlink_update, ConfigurationError, get_latest_checkpoint, save_predictions,\
    save_pred_slt_format, load_scaler
...
class TrainManager:
    ...
    def __init__(self, model: Model, config: dict, test=False, ckpt=None) -> None:
        ...
        # Load scalers to inverse transformation
        self.scaler = None
        if len(config['data']['features']) > 1:
            if config['data']['features'][1] == "face2D":
                self.scaler = load_scaler(
                    config['data']['train'] + '.openface2D_original')
            if config['data']['features'][1] == "face3D":
                self.scaler = load_scaler(
                    config['data']['train'] + '.openface3D_original')
        ...
        self.scheduler, self.scheduler_step_at = build_scheduler(
            ...
            hidden_size=config["model"]["encoder1"]["hidden_size"])
        ...
        self.eval_batch_size = train_config.get(
            "eval_batch_size", self.batch_size)
        self.eval_batch_type = train_config.get(
            "eval_batch_type", self.batch_type)
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) \
        ...
        for epoch_no in range(self.epochs):
            ...
            if self.gaussian_noise:
                if len(all_epoch_noise) != 0:
                    self.model.out_stds = torch.mean(torch.stack(
                        ([noise.std(dim=[0]) for noise in all_epoch_noise])), dim=-2)
                else:
            ...
            for batch in iter(train_iter):
                ...
                batch_loss, noise = self._train_batch(
                    batch, update=update, epoch_num=epoch_no)
                # If Gaussian Noise, collect the noise
                if self.gaussian_noise:
                    # If future Prediction, cut down the noise size to just one frame
                    if self.future_prediction != 0:
                        all_epoch_noise.append(
                            noise.reshape(-1, self.model.out_trg_size // self.future_prediction))
                    else:
                        all_epoch_noise.append(
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    if self.is_best(ckpt_score):
                        ...
                        # save best validation predictions which will be converted to SLT format during testing
                        save_predictions(self.config, valid_hypotheses, 'dev')
                        ...
                        display = list(range(0, len(valid_hypotheses), int(
                            np.ceil(len(valid_hypotheses) / 13.15))))
     ...
     # Save PT predictions in SLT format for final backtranslation evaluation
    def save_predictions_slt_format(self, hypotheses, type):
        slt_cfg_file = self.config["slt"]["config"]
        slt_cfg = load_config(slt_cfg_file)
        slt_model = slt_cfg["training"]["model_dir"].split('/')[-1]
        output_path = os.path.join(
            self.config["training"]["model_dir"], 'SLT_output')

        # Save data in skels format
        save_predictions(self.config, hypotheses, type)

        # Save data in slt format
        data_path = self.config["data"][type]
        extension = ".pred." + self.config["data"]["trg"]
        feature_size = slt_cfg["data"]["feature_size"]
        output_file = os.path.join(os.path.abspath(
            output_path), slt_model) + '.' + type
    ...
    def produce_validation_video(self, output_joints, inputs, references, display, model_dir, type, steps="", file_paths=None):
        ...
        for i in display:
            ...
            timing_hyp_seq, ref_seq_count, dtw_score = alter_DTW_timing(
                seq, ref_seq)

            # video_ext = "{}_{}.mp4".format(file_paths[i].split('/')[-1], "{0:.2f}".format(float(dtw_score)).replace(".", "_"))
    ...
    # Train the batch
    def _train_batch(self, batch: Batch, update: bool = True, epoch_num=0) -> Tensor:

        # Get loss from this batch
        batch_loss, noise = self.model.get_loss_for_batch(
            batch=batch, loss_function=self.loss, epoch_num=self.steps)
...
def train(cfg_file: str, ckpt=None) -> None:
    ...
    train_data, dev_data, test_data, src1_vocab, src2_vocab, trg_vocab = load_data(
        cfg=cfg)

    # Build the Progressive Transformer model
    model = build_model(cfg, src1_vocab=src1_vocab,
                        src2_vocab=src2_vocab,  trg_vocab=trg_vocab)
    ...
    test(cfg_file, ckpt, trainer.logger)

# pylint: disable-msg=logging-too-many-args


def test(cfg_file, ckpt, logger=None) -> None:

    # Load the config file
    cfg = load_config(cfg_file)

    if logger == None:
        logger = make_logger(cfg["training"]["model_dir"])
    ...
    for data_set_name, data_set in data_to_predict.items():
        with open("pred_%s_%s.skels" % (data_set_name, cfg["training"]["model_dir"][9:]), "w") as out_skels:
            ...
            score, loss, references, hypotheses, \
                ...
                validate_on_data(
                    ...
                    type="test" if not data_set_name is "train" else "train_inf",
                    logger=logger,
                    config=cfg
                )
...
#     #     #     #     # train(cfg_file=args.config)
    test(cfg_file="./Configs/Base_train_all.yaml",
         ckpt='./Models/Base_train_dynamic_2gloss_BCD_hard_smaller/580000_every.ckpt')
